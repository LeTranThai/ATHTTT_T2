...
public class Cer {
    ...
    public static InputStream signBill(InputStream billInputStream, InputStream keyStoreInputStream,
    ...
    public static InputStream generateAndLoadKeyStore2(String password, String urFullname, String orgName,
                                                       String orgUnit, String city, String state, String countryCode) {
        try {
            BouncyCastleProvider provider = new BouncyCastleProvider();
            Security.addProvider(provider);
            // Generate Key Pair
            ...
            X509Certificate[] cert = generateCertificate(keyPair, urFullname, orgUnit, orgName, city, state,
                    countryCode);
    ...
    private static X509Certificate[] generateCertificate(KeyPair keyPair, String urFullname, String orgUnit,
                                                         String orgName, String city, String state, String countryCode)
            throws GeneralSecurityException, IOException {
        BouncyCastleProvider provider = new BouncyCastleProvider();
        Security.addProvider(provider);
        X509V3CertificateGenerator certGen = new X509V3CertificateGenerator();
        X500Principal dnName = new X500Principal("CN=" + urFullname + ", OU=" + orgUnit + ", O=" + orgName + ", L="
    ...
    public static Map<String, String> viewCertificateDetails(InputStream keystoreInputStream, String pass) {
        ...
        try {
            ...
            if (certificate instanceof X509Certificate) {
                ...
                certificateDetails.put("Certificate Extensions",
                        Arrays.toString(x509Certificate.getExtensionValue(" ")));
                certificateDetails.put("Certificate Basic Constraints",
                        String.valueOf(x509Certificate.getBasicConstraints()));
                certificateDetails.put("Certificate Key Usage", String.valueOf(x509Certificate.getKeyUsage()));
                certificateDetails.put("Certificate Extended Key Usage",
                        String.valueOf(x509Certificate.getExtendedKeyUsage()));
                certificateDetails.put("Certificate Subject Alternative Names",
                        String.valueOf(x509Certificate.getSubjectAlternativeNames()));
                certificateDetails.put("Certificate Subject Unique ID",
                        String.valueOf(x509Certificate.getSubjectUniqueID()));
                certificateDetails.put("Certificate Issuer Unique ID",
                        String.valueOf(x509Certificate.getIssuerUniqueID()));
                ...
                certificateDetails.put("Certificate Signature Algorithm Parameters",
                        x509Certificate.getSigAlgParams().toString());
    ...
    public static Map<String, String> verifyPdfSignedIntegrity(InputStream resource)
        ...
        for (String name : names) {
            ...
            try {
                ...
                CertificateInfo.X500Name subjectFields = CertificateInfo
                        .getSubjectFields(pkcs7.getSigningCertificate());
                ...
                map.put("signDate",
                        pkcs7.getSignDate().get(Calendar.YEAR) + "-" + (pkcs7.getSignDate().get(Calendar.MONTH) + 1)
    ...
    public InputStream sign(InputStream billPath, String keyStorePassword, InputStream keyStoreData) {
        try {
            ...
            Rectangle rect = new Rectangle(36, 250, 200, 100);
            PdfSignatureAppearance appearance = signer.getSignatureAppearance();
            appearance.setReason("Bill Digital Signature").setLocation("Hochiminh City").setReuseAppearance(false)
    ...
    public static InputStream changePasswordKeyStore(InputStream keystoreInputStream, String oldPassword,
                                                     String newPassword) {
        try {
            Security.addProvider(new BouncyCastleProvider());
            // Load the keystore
            ...
            KeyStore.Entry entry = new KeyStore.PrivateKeyEntry(decryptedPrivateKey,
    ...
    public static InputStream exportCSR(InputStream jksInputStream, String password) {
        try {
            ...
            String alias = "shopphone"; // Change this to your alias
            KeyPair keyPair = getKeyPair(ks, alias, password);
            X509Certificate cert = (X509Certificate) ks.getCertificate(alias);

            // Create PKCS10 Certification Request
            org.bouncycastle.pkcs.PKCS10CertificationRequest csr = generateCSR(keyPair, cert.getSubjectX500Principal());

            // Convert CSR to InputStream
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try (JcaPEMWriter pemWriter = new JcaPEMWriter(new OutputStreamWriter(baos))) {
    ...
    private static KeyPair getKeyPair(KeyStore ks, String alias, String password) throws Exception {
    ...
    private static org.bouncycastle.pkcs.PKCS10CertificationRequest generateCSR(KeyPair keyPair,
                                                                                X500Principal subjectDN) throws IOException, OperatorCreationException {
    ...
    public static void main(String[] args) throws IOException, GeneralSecurityException {
        boolean b = Cer.checkDigitalSignPDF(new FileInputStream("/home/huudan/Downloads/bill-signed19.pdf"), "sig");
